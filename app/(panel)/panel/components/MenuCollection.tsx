        ...
        <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-col  pr-2 '>
                {items.filter(i => i.isMenuItem && i.route.includes(baseLink)).map(item => <Link href={'/panel/'+item.route}
                    className={pathname.includes(item.route) ? 'text-coral bg-orange-200 p-1 text-body-3-bolder border-r border-r-coral' : 'p-1 text-ultra-violet text-body-3-normal hover:text-coral border-r'}
                >
                    {item.title}
                </Link>)}
            </div>
