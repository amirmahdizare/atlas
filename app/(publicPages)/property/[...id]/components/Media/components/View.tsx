...
export const View = ({ src, onClick , isOpen}: { src: string, onClick?: Function , isOpen:boolean }) => {
    ...
    if (isFileSrcImage(finalSrc))
        return <img src={finalSrc} onClick={() => onClick ? onClick() : undefined} className='cursor-pointer max-h-[80vh]  duration-100 translate-all rounded w-full lg:max-w-[60vw] aspect-video  object-cover bg- seasalt b order border-anti-flash-white-lighter' />
