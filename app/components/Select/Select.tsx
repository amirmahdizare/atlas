...
export const Select = ({ fullWidth, items, onChange, value, label, error, placeHolder }: SelectProps) => {
    ...
    return (
        ...
        ${fullWidth ? 'flex-1' : ''}`} onClick={() => setIsOpen(true)}>
            ...
            text-french-gray
            focus:bg-white         
              ${isOpen || value ? 'border-ultra-violet ' : 'border-transparent'}
              transition-all
             [&:not(:placeholder-shown)]:text-space-codet
             border
             cursor-pointer
            ${error ? 'border-bittersweet bg-white' : '    [&:not(:focus):placeholder-shown]:border-anti-flash-white-lighter bg-seasalt'}`}>
