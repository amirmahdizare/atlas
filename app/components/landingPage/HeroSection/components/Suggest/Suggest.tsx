...
export const Suggest = async () => {
    ...
    try {
        const response = await fetch((ApiBaseURL ?? '')?.concat(PropretyEndPoints.SEARCH), { method: 'POST' ,   headers: {
            "Content-Type": "application/json",
          }, body: JSON.stringify({ isSuggested: true, page: 1, limit: 10 }) })

        const data: Array<PropertyDetailType> = await response.json()
